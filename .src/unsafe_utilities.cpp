...
namespace jluna {
    unsafe::Symbol* operator""_sym(const char* str, uint64_t) { return jl_symbol(str); }

    unsafe::Value* operator""_eval(const char* str, uint64_t) { return jl_eval_string(str); }
}

// jluna_m(new_complex_fn)
...
namespace jluna::unsafe {
    ...
    unsafe::Function* get_function(unsafe::Symbol* module_name, unsafe::Symbol* function_name) {
        return unsafe::get_function(
            (unsafe::Module*)jl_get_global(jl_main_module, module_name), function_name
        );
    }
    ...
    unsafe::Value* get_value(unsafe::Symbol* module_name, unsafe::Symbol* name) {
        return unsafe::get_function(
            (unsafe::Module*)jl_get_global(jl_main_module, module_name), name
        );
    }
    ...
    void gc_release(uint64_t id) {
        gc_pause;
        detail::gc_init();
        static unsafe::Value* delete_from_heap
            = get_value(jl_main_module, "__jluna_delete_from_heap"_sym);
        call(delete_from_heap, jl_box_uint64(id));
        gc_unpause;
    }

    void gc_enable() { jl_gc_enable(true); }

    void gc_disable() { jl_gc_enable(false); }

    bool gc_is_enabled() { return jl_gc_is_enabled(); }

    uint64_t get_array_size(unsafe::Array* array) { return array->length; }
    ...
    void sizehint(unsafe::Array* arr, uint64_t n_elements) { jl_array_sizehint(arr, n_elements); }
    ...
    void resize_array(unsafe::Array* array, uint64_t one_d) {
        static unsafe::Function* array_value_t
            = unsafe::get_function("jluna"_sym, "get_value_type_of_array"_sym);
        ...
        if (jl_array_ndims(array) != 1) {
            ...
            auto* tuple = jl_new_struct(
                (jl_datatype_t*)tuple_type, jl_box_int64(static_cast<Int64>(one_d))
            );
#else
            ...
            auto* res = jl_reshape_array(
                jl_apply_array_type(unsafe::call(array_value_t, array), 1), array, tuple
            );
    ...
    void resize_array(unsafe::Array* array, uint64_t one_d, uint64_t two_d) {
        ...
        if (jl_array_ndims(array) != 2) {
            ...
            auto* tuple = jl_new_struct(
                ...
                jl_box_int64(static_cast<Int64>(one_d)),
                jl_box_int64(static_cast<Int64>(two_d))
            );
#else
            auto* tuple = jl_new_struct(
                tuple_type,
            ...
            auto* res = jl_reshape_array(
                jl_apply_array_type(unsafe::call(array_value_t, array), 2), array, tuple
            );
            override_array(array, res);
            gc_unpause;
            return;
