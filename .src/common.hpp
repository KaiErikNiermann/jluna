...
namespace jluna::detail {
    ...
    inline unsafe::DataType* array_value_type(unsafe::Array* array) {
        static auto* get_value_type_of_array
    ...
    inline void assert_type(unsafe::DataType* type_a, unsafe::DataType* type_b) {
        if (not(jl_subtype((jl_value_t*)type_a, (jl_value_t*)type_b)
                or jl_types_equal((jl_value_t*)type_a, (jl_value_t*)type_b))) {
            std::stringstream str;
            str << "Assertion failed: Value is of wrong type. Expected: "
                << detail::to_string((jl_value_t*)type_b);
