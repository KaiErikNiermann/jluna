...
namespace jluna {
    template <
        typename Function_t,
        typename... Args_t,
        std::enable_if_t<std::is_void_v<std::invoke_result_t<Function_t, Args_t...>>, bool>>
    ...
    template <
        typename Function_t,
        typename... Args_t,
        std::enable_if_t<not std::is_void_v<std::invoke_result_t<Function_t, Args_t...>>, bool>>
    static unsafe::Value* box_function_result(Function_t f, Args_t... args) {
        return box(f(args...));
    }
    ...
    unsafe::Value* box(T value) {
        gc_pause;
        auto* array = unsafe::new_array_from_data(
            (unsafe::Value*)as_julia_type<char>::type(), value.data(), value.size()
        );
    ...
    unsafe::Value* box(T value) {
        ...
        auto* array           = unsafe::new_array_from_data(
            (unsafe::Value*)as_julia_type<char>::type(), as_string.data(), as_string.size()
        );
        auto* out = jl_array_to_string(array);
        gc_unpause;
        return out;
    ...
    template <
        typename T,
        typename Value_t,
        std::enable_if_t<std::is_same_v<T, std::complex<Value_t>>, bool>>
    unsafe::Value* box(T value) {
    ...
    template <
        typename T,
        typename Value_t,
        std::enable_if_t<std::is_same_v<T, std::vector<Value_t>>, bool>>
    ...
    template <
        ...
        std::enable_if_t<
            std::is_same_v<T, std::unordered_map<Key_t, Value_t>>
                or std::is_same_v<T, std::map<Key_t, Value_t>>,
            bool>>
    unsafe::Value* box(const T& value) {
        ...
        auto* out = unsafe::call(
            new_dict,
            as_julia_type<Key_t>::type(),
            as_julia_type<Value_t>::type(),
            box((uint64_t)value.size())
        );
        for (auto& pair : value)
    ...
    template <
        typename T,
        typename Value_t,
        std::enable_if_t<std::is_same_v<T, std::set<Value_t>>, bool>>
    unsafe::Value* box(const T& value) {
        ...
        auto* out
            = unsafe::call(new_set, as_julia_type<Value_t>::type(), box((uint64_t)value.size()));
    ...
    template <
        ...
        typename T1,
        typename T2,
        std::enable_if_t<std::is_same_v<T, std::pair<T1, T2>>, bool>>
    unsafe::Value* box(const T& value) {
...
#    pragma warning(disable : 4267)
    ...
    unsafe::Value* box(const T& value) {
...

        {
            uint64_t i = 0;
            std::apply(
                [&](auto... elements) { (jl_arrayset(args_v, box(elements), i++), ...); }, value
            );
        }
        ...
        auto* out
            = jl_new_structv((jl_datatype_t*)tuple_t, (jl_value_t**)args_v->data, args_v->length);
#else
