...
#    pragma warning(disable : 4267)
    ...
    static inline auto fn_buffer(unsafe::Function* fn, Args_t... args) {
        alignas(unsafe::Value*) static thread_local unsafe::Value* buffer[sizeof...(Args_t) + 1];
        return new (buffer)
            unsafe::Value* [sizeof...(Args_t) + 1] { (unsafe::Value*)fn, (unsafe::Value*)args... };
    }

    template <typename... Args_t>
    static inline auto arg_buffer(Args_t... args) {
        alignas(unsafe::Value*) static thread_local unsafe::Value* buffer[sizeof...(Args_t)];
        return new (buffer) unsafe::Value* [sizeof...(Args_t)] { (unsafe::Value*)args... };
    }

    template <is_julia_value_pointer... Args_t>
    inline unsafe::Value* call(Function* function, Args_t... args) {
        return jl_call(function, arg_buffer(args...), sizeof...(Args_t));
    }
...
#    pragma warning(pop)
        ...
        inline std::nullptr_t gc_init() {
            ...
            jl_eval_string(R"(
                if !isdefined(Main, :__jluna_heap) 
                    ...
                    function __jluna_add_to_heap(ptr::UInt64) ::UInt64
                    ...
                    function __jluna_delete_from_heap(ptr::UInt64) ::Nothing
                        lock(__jluna_heap_lock)
    ...
    uint64_t gc_preserve(T* in) {
        ...
        static unsafe::Function* jluna_add_to_heap
            = get_function(jl_main_module, "__jluna_add_to_heap"_sym);
    ...
    unsafe::Array* new_array(unsafe::Value* value_type, Dims... size_per_dimension) {
        ...
        auto* tuple
            = jl_new_struct((jl_datatype_t*)tuple_type, jl_box_uint64(size_per_dimension)...);
#else
    ...
    unsafe::Array*
    new_array_from_data(unsafe::Value* value_type, void* data, Dims... size_per_dimension) {
        bool before = jl_gc_is_enabled();
        jl_gc_enable(false);
        static auto* tuple_type = [&]() {
            std::array<jl_value_t*, sizeof...(Dims)> types;
            for (uint64_t i = 0; i < sizeof...(Dims); ++i)
                types.at(i) = (jl_value_t*)jl_uint64_type;

            return jl_apply_tuple_type_v(types.data(), sizeof...(Dims));
        }();
        ...
        auto* res
            = jl_ptr_to_array(jl_apply_array_type(value_type, sizeof...(Dims)), data, tuple, 0);
        jl_gc_enable(before);
        return res;
    }
    ...
    void resize_array(unsafe::Array* array, Dims... size_per_dimension) {
        ...
        static jl_function_t* get_value_type_of_array
            = get_function("jluna"_sym, "get_value_type_of_array"_sym);
        ...
        auto* res = jl_reshape_array(
            jl_apply_array_type(unsafe::call(get_value_type_of_array, array), sizeof...(Dims)),
            array,
            tuple
        );
    ...
    unsafe::Value* unsafe_box(const T& in) {
        return jl_array_to_string(jl_ptr_to_array_1d(
            jl_apply_array_type((jl_value_t*)jl_char_type, 1), in.data(), in.size(), 0
        ));
    }
