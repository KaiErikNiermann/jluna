...
void jluna_free_lambda(void* in, int n_args) {
    if (n_args == 0)
        ...
        std::cerr << "[C++][WARNING] In c_adapter::jluna_free_lambda: Unreachable reached"
                  << std::endl;
}
...
jluna::unsafe::Value* jluna_invoke_lambda_3(
    void* function_ptr,
    jluna::unsafe::Value* x,
    jluna::unsafe::Value* y,
    jluna::unsafe::Value* z
) {
    return (*reinterpret_cast<jluna::detail::lambda_3_arg*>(function_ptr))(x, y, z);
}

void* jluna_to_pointer(jl_value_t* in) { return (void*)in; }
