...
int main() {
    ...
    Test::test("simple object implementation", []() {
        Usertype<Basic>::initialize_type(TL<Lens<number, &Basic::number>>(), TL<>());
        Usertype<Basic>::implement();
        Test::assert_that(Usertype<Basic>::is_implemented());
        Test::assert_that(Usertype<Basic>::is_abstract() == false);
    });

    Test::test("derived object implementation", []() {
        Usertype<A>::implement();
        Usertype<B>::initialize_type(
            TL<Lens<number, &B::number>, Lens<ptr, &B::ptr>>(), TL<B, C>()
        );
        Usertype<C>::initialize_type(TL<Lens<number, &C::number>>(), TL<>());
        Usertype<C>::implement<A>();
        Usertype<B>::implement<A>();

        Test::assert_that(Usertype<B>::_type_dispatch_map.contains({ptr, "C"}));
        Test::assert_that(Usertype<C>::_type_dispatch_map.contains({number, "Int32"}));
        Test::assert_that(Usertype<B>::_type_dispatch_map.contains({ptr, "B"}));
        Test::assert_that(Usertype<B>::_type_dispatch_map.contains({number, "Int32"}));
        Test::assert_that(Usertype<B>::is_implemented());
        Test::assert_that(Usertype<C>::is_implemented());
        Test::assert_that(Usertype<B>::is_abstract() == false);
        Test::assert_that(Usertype<A>::is_abstract() == true);
        Test::assert_that(Usertype<A>::is_implemented());
    });

    Test::test("derived object box unbox", []() {
        auto b   = B();
        b.number = 42;

        auto c_ptr    = std::make_shared<C>();
        c_ptr->number = 12; // Initialize the derived type's property

        b.ptr = c_ptr;

        auto boxed   = Usertype<B>::box(b);
        auto unboxed = Usertype<B>::unbox(boxed);

        Test::assert_that(b.number == unboxed.number);

        if (auto* derived = dynamic_cast<C*>(unboxed.ptr.get())) {
            Test::assert_that(derived->number == c_ptr->number);
        }
    });

    Test::test("derived object nested unbox", []() {
        auto b   = B();
        b.number = 42;
        ...
        if (auto* derived = dynamic_cast<C*>(dynamic_cast<B*>(unboxed.ptr.get())->ptr.get())) {
            Test::assert_that(derived->number == b_ptr->number);
        }
    });

    Test::test("derived object mapping", []() {
        Usertype<BaseC>::implement();
        Usertype<LeafA>::initialize_type(TL<Lens<name, &LeafA::name>>(), TL<>());
        Usertype<LeafB>::initialize_type(TL<Lens<name, &LeafB::name>>(), TL<>());
        Usertype<Tree>::initialize_type(
            TL<Lens<left, &Tree::left>, Lens<right, &Tree::right>>(),
            TL<Tree, LeafA, LeafB>()
        );
        Usertype<LeafA>::implement<BaseC>();
        Usertype<LeafB>::implement<BaseC>();
        Usertype<Tree>::implement<BaseC>();

        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({left, "LeafA"}));
        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({left, "Tree"}));
        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({left, "LeafB"}));
        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({right, "LeafA"}));
        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({right, "Tree"}));
        Test::assert_that(Usertype<Tree>::_type_dispatch_map.contains({right, "LeafB"}));
    });

    Test::test("from julia derived", []() {
        auto abstracted = Main.safe_eval(R"(
            return Tree(
                Tree(
        ...
        auto unboxed = Usertype<Tree>::unbox(abstracted);

        Test::assert_that(Usertype<LeafB>::is_implemented());
        Test::assert_that(Usertype<LeafA>::is_implemented());
        Test::assert_that(Usertype<Tree>::is_implemented());
        ...
        Test::assert_that(unboxed.foo());
        Test::assert_that(unboxed.left->foo());
        Test::assert_that(unboxed.right->foo());
        if (auto* left = dynamic_cast<Tree*>(unboxed.left.get())) {
            Test::assert_that(left->left->foo());
            Test::assert_that(left->right->foo());
            if (auto* s = dynamic_cast<LeafA*>(left->left.get())) {
                Test::assert_that(s->name == "LeafA");
                Test::assert_that(s->foo());
            }
            if (auto* t = dynamic_cast<LeafB*>(left->right.get())) {
                Test::assert_that(t->name == "LeafB");
                Test::assert_that(t->foo());
            }
        }
        if (auto* right = dynamic_cast<LeafA*>(unboxed.right.get())) {
            Test::assert_that(right->name == "LeafA");
            Test::assert_that(right->foo());
        }
    });

    Test::test("register function derived", []() {
        Main.create_or_assign("foo", as_julia_function<Tree(Tree)>(
        ...
        Main.safe_eval(R"(
            function foo()
                output = foo(Tree(LeafA("LeafA"), LeafB("LeafB")))
            end
