...
namespace jluna {
    ...
    template <typename T>
    concept is_boxable = requires(T t) { box(t); } or requires(T t) { (unsafe::Value*)t; };

    template <typename T>
    concept is_not_boxable = not is_boxable<T>;
    ...
    template <
        typename Function_t,
        typename... Args_t,
        std::enable_if_t<std::is_void_v<std::invoke_result_t<Function_t, Args_t...>>, bool> = true>
